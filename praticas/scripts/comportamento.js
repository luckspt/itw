...
function desenhaTopsSemanais() {
  ...
  // Nota: Esta função não utiliza jQuery e não deve ser alterada para já.

  // Útil para mandar mostrar a lista de músicas de cada top semanal.
  let elementoPai = document.getElementById(SECCAO_TOPS_SEMANAIS);
  ...
  for (let topSemanal of TOPS_SEMANAIS) {
    ...
    pTopSemanal = document.createElement("p");
    pTopSemanal.innerText = "Top da semana " + topSemanal.semana +
                            " do ano " + topSemanal.ano;

    elementoPai.appendChild(pTopSemanal);

    // Mostra a lista de músicas do top semanal atual.
    desenhaListaMusicas(elementoPai, topSemanal.musicas,
                        "semana" + topSemanal.semana + "de" +
                        topSemanal.ano + "-");
  }
}
...
function desenhaTodasMusicas() {
  ...
  desenhaListaMusicas(document.getElementById(SECCAO_TODAS_MUSICAS), MUSICAS);
}
 ...
 * 
 * @param {Element} elementoPai Elemento dentro do qual é mostrada a lista de
 *                              músicas.
...
function desenhaListaMusicas(elementoPai, musicas, prefixo = "") {
  ...
  let olMusicas = document.createElement("ol");
  ...
  for (let musica of musicas) {
    ...
    liMusica = document.createElement("li");
    liMusica.id = prefixo + musica.id;
    liMusica.classList.add(CLASSE_MUSICA);
    liMusica.innerHTML = "<p>" + musicaEmHTML(musica) + "</p>";

    // A lista vai crescendo um item de cada vez.
    olMusicas.appendChild(liMusica);
  }

  // Mostrar a lista ao utilizador.
  elementoPai.appendChild(olMusicas);
}
